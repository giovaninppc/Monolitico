...
// variaveis para tratar dos dados "tratados" do acelerometro
int16_t TAcX, TAcY, TAcZ, TTmp, TGyX, TGyY, TGyZ;

//Constantes de correcao do sensor
int16_t CAcX = 16500;
int16_t CAcY = -80;
int16_t CAcZ = 450;
int16_t CGyX = -245;
...
#define AIO_USERNAME     "giovaninppc"
#define AIO_KEY          "1e3da17e285b4e389be5e571e5ea9c7b"
 
WiFiClient client;
Adafruit_MQTT_Client mqtt(&client, AIO_SERVER, AIO_SERVERPORT, AIO_USERNAME, AIO_KEY);
Adafruit_MQTT_Publish SGyX = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/GyX");
Adafruit_MQTT_Publish SGyY = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/GyY");
Adafruit_MQTT_Publish SGyZ = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/GyZ");
Adafruit_MQTT_Publish STemp = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/Temp");
Adafruit_MQTT_Publish SAcX = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/AcX");
Adafruit_MQTT_Publish SAcY = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/AcY");
Adafruit_MQTT_Publish SAcZ = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/AcZ");
void MQTT_connect();
...
void calibrateValues(){
    ...
    TGyX = ((GyX - CGyX)/250)*250;
    TGyY = ((GyY - CGyY)/250)*250;
    TGyZ = ((GyZ - CGyZ)/250)*250;
    TTmp = (Tmp/340.00+36.53);
    TAcX = ((AcX - CAcX)/250)*250;
    TAcY = ((AcY - CAcY)/250)*250;
    TAcZ = ((AcZ - CAcZ)/250)*250;

  Serial.print("Treated Data:");
  Serial.print("AcX = "); Serial.print(TAcX);
  Serial.print(" | AcY = "); Serial.print(TAcY);
  Serial.print(" | AcZ = "); Serial.print(TAcZ);
  Serial.print(" | Tmp = "); Serial.print(Tmp/340.00+36.53);
  Serial.print(" | GyX = "); Serial.print(TGyX);
  Serial.print(" | GyY = "); Serial.print(TGyY);
  Serial.print(" | GyZ = "); Serial.println(TGyZ);
}

/* funcao que pubica os valores no servidor da Adafruit*/
void publishValues(){
